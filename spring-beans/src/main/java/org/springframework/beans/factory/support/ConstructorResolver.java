...
class ConstructorResolver {
		...
		if (constructorToUse == null || argsToUse == null) {
			...
			if (candidates.length == 1 && explicitArgs == null && !mbd.hasConstructorArgumentValues()) {
				...
				if (uniqueCandidate.getParameterCount() == 0) {
					...
					//<1>设置Bean实例  创建 Bean（三）之实例化 Bean 对象(2) <1>
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		//主要为【构造参数】确认，构造参数的确认主要分为三种情况：explicitArgs 参数、缓存中获取、配置文件中解析
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			if (candidateList == null) {
				...
				// 获取所有待定方法 获取所有的构造方法，同时对构造方法进行筛选
			...
			//排序规则：先按照 public/非public 构造函数升序，再按照构造参数数量降序。
			...
			int minNrOfArgs;
			//如果提供了参数explicitArgs，则直接比较两者长度是否相等，如果相等则表示找到了，否则继续比较
			if (explicitArgs != null) {
				minNrOfArgs = explicitArgs.length;
			}
			else {
				...
				//即没有通过传递参数的方式传递构造参数，缓存中也没有，那就只能通过解析配置文件获取构造参数
				...
				if (paramTypes.length >= minNrOfArgs) {
					...
					if (explicitArgs != null) {
						...
						try {
							...
							// ParameterNameDiscoverer 是用于解析【方法】和【构造函数】的【参数】名称的接口，为参数名称探测器
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							// 获取指定构造函数的参数名称
							if (pnd != null) {
								paramNames = pnd.getParameterNames(candidate);
